...
export default function Sanificatore() {
  const { t, i18n } = useTranslation(undefined, { keyPrefix: "pages.sanitizer" });
      ...
      <Box bg="gray.50" py={{ base: 12, md: 20 }}>
        <Container maxW="6xl">
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={8}>
            {t("principle.cards", { returnObjects: true }).map((card, i) => (
              <Box key={i} p={6} bg="white" rounded="xl" shadow="md">
                <Heading as="h4" size="md" mb={3}>{card.title}</Heading>
      ...
      <Box py={{ base: 12, md: 20 }}>
        <Container maxW="6xl">
          ...
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={8}>
            {t("sectors.groups", { returnObjects: true }).map((group, i) => (
              <Box key={i}>
